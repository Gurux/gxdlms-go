package enums

//
// --------------------------------------------------------------------------
//  Gurux Ltd
//
//
//
// Filename:        $HeadURL$
//
// Version:         $Revision$,
//                  $Date$
//                  $Author$
//
// Copyright (c) Gurux Ltd
//
//---------------------------------------------------------------------------
//
//  DESCRIPTION
//
// This file is a part of Gurux Device Framework.
//
// Gurux Device Framework is Open Source software; you can redistribute it
// and/or modify it under the terms of the GNU General Public License
// as published by the Free Software Foundation; version 2 of the License.
// Gurux Device Framework is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
// See the GNU General Public License for more details.
//
// More information of Gurux products: https://www.gurux.org
//
// This code is licensed under the GNU General Public License v2.
// Full text may be retrieved at http://www.gnu.org/licenses/gpl-2.0.txt
//---------------------------------------------------------------------------

import (
	"fmt"
	"strings"

	"github.com/Gurux/gxcommon-go"
)

// DataProtectionKeyType Enumerates data protection key types.
type DataProtectionKeyType int

const (
	// DataProtectionKeyTypeIdentified defines that the identified key.
	DataProtectionKeyTypeIdentified DataProtectionKeyType = iota
	// DataProtectionKeyTypeWrapped defines that the wrapped key.
	DataProtectionKeyTypeWrapped
	// DataProtectionKeyTypeAgreed defines that the agreed.
	DataProtectionKeyTypeAgreed
)

// DataProtectionKeyTypeParse converts the given string into a DataProtectionKeyType value.
//
// It returns the corresponding DataProtectionKeyType constant if the string matches
// a known level name, or an error if the input is invalid.
func DataProtectionKeyTypeParse(value string) (DataProtectionKeyType, error) {
	var ret DataProtectionKeyType
	var err error
	switch {
	case strings.EqualFold(value, "Identified"):
		ret = DataProtectionKeyTypeIdentified
	case strings.EqualFold(value, "Wrapped"):
		ret = DataProtectionKeyTypeWrapped
	case strings.EqualFold(value, "Agreed"):
		ret = DataProtectionKeyTypeAgreed
	default:
		err = fmt.Errorf("%w: %q", gxcommon.ErrUnknownEnum, value)
	}
	return ret, err
}

// String returns the canonical name of the DataProtectionKeyType.
// It satisfies fmt.Stringer.
func (g DataProtectionKeyType) String() string {
	var ret string
	switch g {
	case DataProtectionKeyTypeIdentified:
		ret = "Identified"
	case DataProtectionKeyTypeWrapped:
		ret = "Wrapped"
	case DataProtectionKeyTypeAgreed:
		ret = "Agreed"
	}
	return ret
}

// AllDataProtectionKeyType returns a slice containing all defined DataProtectionKeyType values.
func AllDataProtectionKeyType() []DataProtectionKeyType {
	return []DataProtectionKeyType{
	DataProtectionKeyTypeIdentified,
	DataProtectionKeyTypeWrapped,
	DataProtectionKeyTypeAgreed,
	}
}
